...
export function parseRecord(content: string, filePath?: string): ParseResult {
  try {
    ...
    const { $schema: _schema, ...withoutSchema } = rawPayload;
    const normalizedPayload = ('id' in withoutSchema && 'recordId' in withoutSchema)
      ? (() => {
          const { recordId: _recordId, ...rest } = withoutSchema;
          return rest;
        })()
      : withoutSchema;

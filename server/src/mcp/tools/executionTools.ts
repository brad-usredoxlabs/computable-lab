...
export function registerExecutionTools(server: McpServer, ctx: AppContext, registry?: ToolRegistry): void {
  ...
  const executionRunService = new ExecutionRunService(ctx, provider, controlService, evidenceService);
  ...
  // execution_run_evidence — list append-only evidence records
  dualRegister(server, registry,
    'execution_run_evidence',
    'List append-only remediation decisions, deviations, and observations for an execution run.',
    {
      executionRunId: z.string().describe('Execution-run recordId'),
    },
    async (args) => {
      try {
        const evidence = await evidenceService.listExecutionEvidence(args.executionRunId);
  ...
  // execution_run_reality — reconstruct intended vs actual execution state
  dualRegister(server, registry,
    'execution_run_reality',
    'Reconstruct compiled intent, accepted diffs, and observed outcomes for an execution run.',
    {
      executionRunId: z.string().describe('Execution-run recordId'),
    },
    async (args) => {
      try {
        const reality = await evidenceService.getExecutionReality(args.executionRunId);
        return jsonResult({ reality });
      } catch (err) {
        if (err instanceof ExecutionError) {
          return errorResult(`${err.code}: ${err.message}`);
        }
  ...
  // execution_run_record_remediation — append remediation acceptance/rejection
  dualRegister(server, registry,
    'execution_run_record_remediation',
    'Record an append-only remediation decision against the active compiled plan.',
    {
      executionRunId: z.string().describe('Execution-run recordId'),
      ...
      }).describe('Compiler-emitted remediation selector'),
      decision: z.enum(['accepted', 'rejected']).describe('Decision outcome'),
      actor: z.object({
        actorId: z.string(),
        role: z.enum(['run-operator', 'supervisor', 'qa-reviewer', 'system']),
        displayName: z.string().optional(),
      }).describe('Actor accepting or rejecting the remediation'),
      rationale: z.string().describe('Human-readable rationale'),
      decidedAt: z.string().optional().describe('Optional timestamp override'),
      acceptedDiff: z.object({
        stepId: z.string().optional(),
        target: z.enum(['protocol-step', 'robot-step', 'run']).optional(),
        path: z.string().optional(),
        op: z.enum(['add', 'replace', 'remove', 'annotate']).optional(),
        previousValue: z.unknown().optional(),
        value: z.unknown().optional(),
        note: z.string().optional(),
      }).optional().describe('Machine-readable diff preserved when the remediation is accepted'),
    },
    async (args) => {
      try {
        const remediation = {
          code: args.remediation.code,
          stepId: args.remediation.stepId,
          ...(args.remediation.action ? { action: args.remediation.action } : {}),
        };
        const actor = {
          actorId: args.actor.actorId,
          role: args.actor.role,
          ...(args.actor.displayName ? { displayName: args.actor.displayName } : {}),
        };
        const acceptedDiff = args.acceptedDiff
          ? {
            ...(args.acceptedDiff.stepId ? { stepId: args.acceptedDiff.stepId } : {}),
            ...(args.acceptedDiff.target ? { target: args.acceptedDiff.target } : {}),
            ...(args.acceptedDiff.path ? { path: args.acceptedDiff.path } : {}),
            ...(args.acceptedDiff.op ? { op: args.acceptedDiff.op } : {}),
            ...(args.acceptedDiff.previousValue !== undefined ? { previousValue: args.acceptedDiff.previousValue } : {}),
            ...(args.acceptedDiff.value !== undefined ? { value: args.acceptedDiff.value } : {}),
            ...(args.acceptedDiff.note ? { note: args.acceptedDiff.note } : {}),
          }
          : undefined;
        const result = await evidenceService.recordRemediationDecision(args.executionRunId, {
          remediation,
  ...
  // execution_run_record_deviation — append deviation record
  dualRegister(server, registry,
    'execution_run_record_deviation',
    'Record an append-only execution deviation or accepted diff without mutating the source plan.',
    {
      executionRunId: z.string().describe('Execution-run recordId'),
      deviationType: z.enum(['remediation', 'operator', 'runtime']).describe('Deviation category'),
      actor: z.object({
        actorId: z.string(),
        role: z.enum(['run-operator', 'supervisor', 'qa-reviewer', 'system']),
        displayName: z.string().optional(),
      }).describe('Actor recording the deviation'),
      rationale: z.string().describe('Rationale for the deviation'),
      status: z.enum(['accepted', 'observed']).optional().describe('Accepted diffs require approval authority'),
      stepId: z.string().optional().describe('Optional affected compiled step'),
      recordedAt: z.string().optional().describe('Optional timestamp override'),
      remediationDecisionId: z.string().optional().describe('Optional accepted remediation decision to chain from'),
      diff: z.object({
        stepId: z.string().optional(),
        target: z.enum(['protocol-step', 'robot-step', 'run']).optional(),
        path: z.string().optional(),
        ...
        value: z.unknown().optional(),
        note: z.string().optional(),
      }).optional().describe('Machine-readable diff payload'),
    },
    async (args) => {
      try {
        const actor = {
        ...
        const diff = args.diff
          ? {
            ...(args.diff.stepId ? { stepId: args.diff.stepId } : {}),
            ...(args.diff.target ? { target: args.diff.target } : {}),
            ...(args.diff.path ? { path: args.diff.path } : {}),
            ...(args.diff.op ? { op: args.diff.op } : {}),
            ...(args.diff.previousValue !== undefined ? { previousValue: args.diff.previousValue } : {}),
            ...(args.diff.value !== undefined ? { value: args.diff.value } : {}),
            ...(args.diff.note ? { note: args.diff.note } : {}),
          }
          : undefined;
        const result = await evidenceService.recordDeviation(args.executionRunId, {
          deviationType: args.deviationType,
          actor,
          rationale: args.rationale,
          ...(args.status ? { status: args.status } : {}),
          ...(args.stepId ? { stepId: args.stepId } : {}),
          ...(args.recordedAt ? { recordedAt: args.recordedAt } : {}),
          ...(args.remediationDecisionId ? { remediationDecisionId: args.remediationDecisionId } : {}),
          ...(diff ? { diff } : {}),
        });
        return jsonResult({ success: true, recordId: result.recordId });
      } catch (err) {
  ...
  // execution_run_record_observation — append actual observed outcome
  dualRegister(server, registry,
    'execution_run_record_observation',
    'Record append-only actual observed execution evidence for a run or step.',
    {
      executionRunId: z.string().describe('Execution-run recordId'),
      observationType: z.enum(['step-outcome', 'runtime-note', 'measurement']).describe('Observation category'),
      actor: z.object({
        actorId: z.string(),
        role: z.enum(['run-operator', 'supervisor', 'qa-reviewer', 'system']),
        displayName: z.string().optional(),
      }).describe('Actor recording the observed outcome'),
      observedOutcome: z.object({
        stepId: z.string().optional(),
        outcomeCode: z.string(),
        status: z.string().optional(),
        details: z.record(z.string(), z.unknown()).optional(),
      }).describe('Machine-readable observed outcome'),
      recordedAt: z.string().optional().describe('Optional timestamp override'),
      stepId: z.string().optional().describe('Optional step id if not embedded in observedOutcome'),
      deviationId: z.string().optional().describe('Optional deviation record to link observed outcome back to'),
    },
    async (args) => {
      try {
        const actor = {
        ...
        const observedOutcome = {
          outcomeCode: args.observedOutcome.outcomeCode,
          ...(args.observedOutcome.stepId ? { stepId: args.observedOutcome.stepId } : {}),
          ...(args.observedOutcome.status ? { status: args.observedOutcome.status } : {}),
          ...(args.observedOutcome.details ? { details: args.observedOutcome.details } : {}),
        };
        const result = await evidenceService.recordObservation(args.executionRunId, {
          observationType: args.observationType,
          actor,
          observedOutcome,
          ...(args.recordedAt ? { recordedAt: args.recordedAt } : {}),
          ...(args.stepId ? { stepId: args.stepId } : {}),
          ...(args.deviationId ? { deviationId: args.deviationId } : {}),
        });
        return jsonResult({ success: true, recordId: result.recordId });
      } catch (err) {
        if (err instanceof ExecutionError) {

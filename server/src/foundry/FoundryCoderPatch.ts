...
'Find the real root cause, then fix it. The provided context is a starting point, but the bug is often in a different pass/file than it first appears — trace the failing output back to the code that produces it before editing. read_file pages large files via offset/limit, so read the relevant code in full rather than guessing. Then make the smallest coherent edit; avoid speculative refactors.',
        ...
        ...(retrievalAvailable
          ? ['', 'Use the retrieve tool to find code by concept or symbol (e.g. "the pass that emits deckLayoutPlan.pinned") before reading or paging large files — it is faster than scanning. IMPORTANT: the pipeline pass files (DeterministicPrecompilePass.ts, ChatbotCompilePasses.ts) are 100-150KB each. NEVER read them whole — the read_file tool will refuse and tell you to use retrieve or paged reads. Always start with retrieve("<symbol or concept>") to find the relevant function/region, then read_file with offset+limit to read just that range.']
          : []),
        ...(fixItTools

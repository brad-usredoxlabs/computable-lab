...
describe('ExecutionOrchestrator', () => {
  ...
  it('creates and compiles planned runs by compiler family', async () => {
    ...
    expect((planned.envelope.payload as { protocolLayer?: string }).protocolLayer).toBe('lab');
    expect((planned.envelope.payload as { protocolCompilation?: { targetLayer?: string } }).protocolCompilation?.targetLayer).toBe('lab');
  ...
  it('compiles manual platforms into operator-facing plans', async () => {
    const orchestrator = new ExecutionOrchestrator(ctx);
    ...
    expect(compiled.robotPlanId).toMatch(/^RP-\d{6}$/);
    const compiledEnvelope = await ctx.store.get(compiled.robotPlanId);
    const compiledPayload = compiledEnvelope?.payload as { status?: string; notes?: string; artifacts?: unknown[] };
    expect(compiledPayload.status).toBe('compiled');
    expect(compiledPayload.notes).toContain('Manual execution plan');
    expect(compiledPayload.artifacts).toEqual([]);

    const emitted = await orchestrator.emitExecutionPlan({
      executionPlanId: 'EPL-000001',
      targetPlatform: manualPlatform,
    });
    const emittedEnvelope = await ctx.store.get(emitted.robotPlanId);
    const emittedPayload = emittedEnvelope?.payload as { status?: string; notes?: string };
    expect(emittedPayload.status).toBe('compiled');
    expect(emittedPayload.notes).toContain('Manual execution plan');
  });
});

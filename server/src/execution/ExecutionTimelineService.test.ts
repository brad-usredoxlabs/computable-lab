...
describe('ExecutionTimelineService', () => {
  ...
  beforeAll(async () => {
    ...
    await ctx.store.create({
      envelope: {
        recordId: 'ERD-000001',
        schemaId: 'https://computable-lab.com/schema/computable-lab/execution-remediation-decision.schema.yaml',
        payload: {
          kind: 'execution-remediation-decision',
          recordId: 'ERD-000001',
          executionRunRef: { kind: 'record', id: 'EXR-000001', type: 'execution-run' },
          ...
          rationale: 'Accepted backend change',
          decidedAt: '2026-01-01T00:00:02.500Z',
          activePolicy: {
            scope: { organizationId: 'org-1', runId: 'EXR-000001' },
            profileIds: ['run-1'],
            approvalAuthority: 'supervisor',
            originProfileId: 'run-1',
            settings: { approvalAuthority: 'supervisor' },
          },
        },
      },
      message: 'seed remediation decision',
      skipValidation: true,
      skipLint: true,
    });
  });
  ...
  it('builds merged timeline from run, logs, and materialized event graph', async () => {
    ...
    expect(timeline.entries.some((entry) => entry.source === 'execution-evidence' && entry.type === 'remediation_accepted')).toBe(true);

...
export class ExecutionPlanningValidator {
  validate(input: {
    ...
    const requiredLabwarePlacements = new Set<string>(evgLabwareIds);
    for (const ev of events) {
      for (const labwareId of evgLabwareIds) {
        if (eventMentionsLabwareId(ev?.details, labwareId)) {
          requiredLabwarePlacements.add(labwareId);
        }
      }
    }

    for (const labwareId of [...requiredLabwarePlacements].sort()) {

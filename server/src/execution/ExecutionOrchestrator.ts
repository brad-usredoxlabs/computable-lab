...
import { ProtocolCompiler, type ProtocolCompilerContext, type ProtocolCompilerResult } from '../compiler/protocol/ProtocolCompiler.js';
...
import {
  ...
  parseOperationTemplatePayload,
  resolveEventGraphOperationTemplates,
...
type PlannedRunPayload = {
  ...
  protocolCompilation?: ProtocolCompilerResult;
};
...
function normalizeBindings(input: unknown): PlannedRunBindings | undefined {
  ...
  const executionPlanRef = obj['executionPlanRef'];
  if (executionPlanRef !== undefined) {
    bindings.executionPlanRef = executionPlanRef;
  }
...
function artifactPathFor(robotPlanId: string, targetPlatform: string, extension: string): string {
...
export class ExecutionOrchestrator {
  ...
  private readonly protocolCompiler: ProtocolCompiler;
  ...
  constructor(
    ...
    this.protocolCompiler = new ProtocolCompiler(ctx.store);
  }
  ...
  async createPlannedRun(input: {
    ...
    sourceRef: unknown;
    bindings?: unknown;
    compilerContext?: ProtocolCompilerContext;
    ...
    const protocolCompilation = input.sourceType === 'protocol'
      ? await this.protocolCompiler.lowerToLabProtocol({
          protocolEnvelope: sourceRecord,
          ...(bindings ? { bindings } : {}),
          ...(input.compilerContext ? { context: input.compilerContext } : {}),
        })
      : undefined;
  ...
  async compilePlannedRun(input: {
    ...
    const payload: RobotPlanPayload = {
      ...
      ...(compiled.compatibilityReport && compiled.compatibilityReport.length > 0
        ? { compatibilityReport: compiled.compatibilityReport }
        : {}),
    };
  ...
  private async compileForTarget(input: {
    ...
    const compilerFamily = this.resolveCompilerFamily(input.targetPlatform);
    const resolvedEventGraph = input.eventGraph
      ? await this.resolveTemplateBackedEventGraph(input.eventGraph)
      : null;
    const eventGraph = resolvedEventGraph?.eventGraph ?? input.eventGraph;
    const templateReports = resolvedEventGraph?.reports ?? [];

    if (compilerFamily === 'opentrons') {
      if (input.targetPlatform !== 'opentrons_ot2' && input.targetPlatform !== 'opentrons_flex') {
        throw new ExecutionError(
          'COMPILER_PLATFORM_UNSUPPORTED',
          `Compiler family "opentrons" does not support platform "${input.targetPlatform}" yet`,
          400,
        );
      }
      ...
      return {
        ...
        compatibilityReport: [...templateReports, ...(compiled.compatibilityReport ?? [])],
      };
    }

    if (compilerFamily === 'assist_plus') {
    ...
    if (compilerFamily === 'manual') {
      const { compileManualPlan } = await import('./compilers/manualCompiler.js');
      const compiled = compileManualPlan({
        robotPlanId: input.robotPlanId,
        ...(eventGraph ? { eventGraph } : {}),
        ...(input.executionPlan ? { executionPlan: input.executionPlan } : {}),
      });
    ...
    throw new ExecutionError(
      ...
      `No compiler is configured for platform "${input.targetPlatform}"`,
      400,
    );
  }
  ...
  private resolveCompilerFamily(targetPlatform: string): string {
    const platform = this.ctx.platformRegistry.getPlatform(targetPlatform);
    if (!platform) {
  ...
  private async resolveTemplateBackedEventGraph(eventGraph: EventGraphPayload): Promise<{
    eventGraph: EventGraphPayload;
    reports: CompatibilityReportEntry[];
  }> {
    return resolveEventGraphOperationTemplates(eventGraph, async (templateId) => {
      const envelope = await this.ctx.store.get(templateId);
      if (!envelope || envelope.schemaId !== OPERATION_TEMPLATE_SCHEMA_ID) return null;
      return parseOperationTemplatePayload(envelope.payload, envelope.recordId);
    });
  }

  private mapDerivedArtifactTarget(targetPlatform: string): DerivedArtifactTarget {
    const compilerFamily = this.resolveCompilerFamily(targetPlatform);
    if (compilerFamily === 'assist_plus') return 'pyalab';

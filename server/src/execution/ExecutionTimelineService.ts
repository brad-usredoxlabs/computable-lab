...
type TimelineEntry = {
  at: string;
  source: 'execution-run' | 'instrument-log' | 'event-graph' | 'execution-evidence';
  type: string;
  message: string;
...
export class ExecutionTimelineService {
  ...
  private readonly evidence: ExecutionEvidenceService;

  constructor(ctx: AppContext, runs?: ExecutionRunService, evidence?: ExecutionEvidenceService) {
    this.ctx = ctx;
    this.runs = runs ?? new ExecutionRunService(ctx);
    this.evidence = evidence ?? new ExecutionEvidenceService(ctx);
  }
  ...
  async getTimeline(executionRunId: string): Promise<{ executionRunId: string; entries: TimelineEntry[]; total: number }> {
    ...
    const evidence = await this.evidence.listExecutionEvidence(executionRunId);
    for (const decision of evidence.remediationDecisions) {
      entries.push({
        at: decision.decidedAt,
        source: 'execution-evidence',
        type: `remediation_${decision.decision}`,
        message: `${decision.proposedRemediation.action} ${decision.decision}`,
        data: {
          recordId: decision.recordId,
          stepId: decision.proposedRemediation.stepId,
    ...
    for (const deviation of evidence.deviations) {
      entries.push({
        at: deviation.recordedAt,
        source: 'execution-evidence',
        type: 'deviation_recorded',
        message: `${deviation.deviationType} deviation ${deviation.status}`,
        data: {
          recordId: deviation.recordId,
          stepId: deviation.compiledIntent.stepId ?? deviation.diff.stepId,
          path: deviation.diff.path,
    ...
    for (const observation of evidence.observations) {
      entries.push({
        ...
        message: observation.observedOutcome.outcomeCode,
        data: {
          recordId: observation.recordId,
          stepId: observation.compiledIntent?.stepId ?? observation.observedOutcome.stepId,

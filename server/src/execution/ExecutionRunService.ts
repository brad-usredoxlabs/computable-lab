...
export class ExecutionRunService {
  ...
  constructor(ctx: AppContext, provider?: ExecutionProvider, control?: ExecutionControlService, evidence?: ExecutionEvidenceService) {
    this.ctx = ctx;
  ...
  async listExecutionEvidence(executionRunId: string): Promise<{
    remediationDecisions: unknown[];
    deviations: unknown[];
    observations: unknown[];
    total: number;
  }> {
    return this.evidence.listExecutionEvidence(executionRunId);
  }

  async getExecutionReality(executionRunId: string): Promise<Record<string, unknown>> {
    return this.evidence.getExecutionReality(executionRunId) as unknown as Record<string, unknown>;
  }
}

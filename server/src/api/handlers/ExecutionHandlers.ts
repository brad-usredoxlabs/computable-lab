...
export function createExecutionHandlers(ctx: AppContext) {
  ...
  return {
    ...
    /**
     * GET /execution-runs/:id/evidence
     * List append-only remediation, deviation, and observation records for an execution run.
     */
    async getExecutionRunEvidence(
      request: FastifyRequest<{
        Params: { id: string };
      }>,
      reply: FastifyReply,
    ): Promise<{ evidence: Record<string, unknown> } | ApiError> {
      try {
        const evidence = await evidenceService.listExecutionEvidence(request.params.id);
        return { evidence: evidence as unknown as Record<string, unknown> };
      } catch (err) {
        if (err instanceof ExecutionError) {
          reply.status(err.statusCode);
          return { error: err.code, message: err.message };
    ...
    /**
     * GET /execution-runs/:id/reality
     * Reconstruct intended compiled state, accepted diffs, and observed outcomes.
     */
    async getExecutionRunReality(
      request: FastifyRequest<{
        Params: { id: string };
      }>,
      reply: FastifyReply,
    ): Promise<{ reality: Record<string, unknown> } | ApiError> {
      try {
        const reality = await evidenceService.getExecutionReality(request.params.id);
        return { reality: reality as unknown as Record<string, unknown> };
      } catch (err) {
        if (err instanceof ExecutionError) {
          reply.status(err.statusCode);
          return { error: err.code, message: err.message };
    ...
    /**
     * POST /execution-runs/:id/remediation-decisions
     * Record an append-only remediation decision referencing a compiler proposal.
     */
    async recordExecutionRunRemediationDecision(
      request: FastifyRequest<{
        Params: { id: string };
        Body: {
          remediation: { code: string; stepId: string; action?: string };
          decision: 'accepted' | 'rejected';
          actor: { actorId: string; role: 'run-operator' | 'supervisor' | 'qa-reviewer' | 'system'; displayName?: string };
          rationale: string;
          decidedAt?: string;
          acceptedDiff?: {
            stepId?: string;
            target?: 'protocol-step' | 'robot-step' | 'run';
            path?: string;
            op?: 'add' | 'replace' | 'remove' | 'annotate';
      ...
      try {
        const result = await evidenceService.recordRemediationDecision(request.params.id, request.body);
    ...
    /**
     * POST /execution-runs/:id/deviations
     * Record an append-only deviation/diff without mutating the compiled plan.
     */
    async recordExecutionRunDeviation(
      request: FastifyRequest<{
        Params: { id: string };
        Body: {
      ...
      try {
        const result = await evidenceService.recordDeviation(request.params.id, request.body);
        return { success: true, recordId: result.recordId };
      } catch (err) {
        if (err instanceof ExecutionError) {
          reply.status(err.statusCode);
    ...
    /**
     * POST /execution-runs/:id/observations
     * Record an append-only observed execution outcome.
     */
    async recordExecutionRunObservation(
      request: FastifyRequest<{
        Params: { id: string };
        Body: {
      ...
      try {
        const result = await evidenceService.recordObservation(request.params.id, request.body);
        return { success: true, recordId: result.recordId };
      } catch (err) {
        if (err instanceof ExecutionError) {
          reply.status(err.statusCode);

...
describe('Prompt 04 - debug 8', () => {
  it('debug pipeline run with expand_biology_verbs', async () => {
    ...
    console.log('expand_biology_verbs output:', JSON.stringify(result.outputs.get('expand_biology_verbs'), null, 2));
    console.log('resolve_roles output events count:', (result.outputs.get('resolve_roles') as { events?: unknown[] })?.events?.length);
    console.log('total events in result:', result.outputs.get('resolve_roles') as { events?: unknown[] });

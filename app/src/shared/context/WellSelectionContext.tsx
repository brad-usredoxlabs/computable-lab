/**
 * WellSelectionContext - per-plate well-grid selection state.
 *
 * Drives interactive selection on a single labware (the well grid widget).
 * Distinct from the cross-endpoint `SelectionContext`, which tracks the
 * higher-level source/target selections published by editors and consumed
 * by the slash menu's `/s` and `/t` lookups.
 */
 ...
 */
interface WellSelectionContextValue {
...
const WellSelectionContext = createContext<WellSelectionContextValue | null>(null)
...
/**
 * Props for WellSelectionProvider
 */
interface WellSelectionProviderProps {
  children: ReactNode
  plateConfig: PlateConfig
}

/**
 * WellSelectionProvider - Provides selection context to children
 */
export function WellSelectionProvider({ children, plateConfig }: WellSelectionProviderProps) {
  ...
  const value: WellSelectionContextValue = {
    state,
    selectWell,
    selectWells,
    deselectWell,
  ...
  return (
    <WellSelectionContext.Provider value={value}>
      {children}
    </WellSelectionContext.Provider>
  )
}
 ...
 */
export function useWellSelection(): WellSelectionContextValue {
  const context = useContext(WellSelectionContext)
  if (!context) {
    throw new Error('useWellSelection must be used within a WellSelectionProvider')
  }
  return context
}

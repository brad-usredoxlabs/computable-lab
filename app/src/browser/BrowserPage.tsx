/**
 * `/browser` — schema-driven, JSON-LD-backed record browser.
 *
 * The single destination for everything that has ever been recorded. Type,
 * query, facets, and selection live in URL params (`?type=…&q=…&id=…&facet.X=Y`).
 * Three regions:
 * - Sidebar: SchemaTree (every record kind, grouped by domain).
 * - Centre:  SearchBar + ResultsTable (columns from each schema's ui.yaml).
 * - Detail:  edit-in-place via the projection TipTap editor.
 *
 * Mounts under AppShell with brand="Records", endpoint-scoped AI dock,
 * and the shared four-endpoint NavLinks.
 */
...
function BrowserPageInner() {
  ...
  // Cursor history — Phase 1's index returns opaque cursors; to support
  // "Prev" we keep the trail in component state. Cleared on any non-cursor
  // change to the query.
  useEffect(() => {
    setPrevCursors([])
  ...
  return (
    <AppShell
      brand="Records"
      topbarMiddle={
        <span className="cl-browser__breadcrumb">
          {state.type ? state.type : 'All records'}
        </span>
      }
